use super::{mock::*, Entity, EntityProperty, Error, Event as ZeroEvent, Sense, SenseREP, SenseTrust, SenseXP};
...
fn sense_create_entity() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ZeroSense::create_entity(RawOrigin::Root.into(), account, cid.clone()));
		...
		assert_eq!(EntityProperty::new(0, block_number), ZeroSense::rep(account));
		assert_eq!(EntityProperty::new(0, block_number), ZeroSense::trust(account));
